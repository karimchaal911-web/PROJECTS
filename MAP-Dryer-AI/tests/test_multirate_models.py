...
class SavedArtifactTests(unittest.TestCase):
    ...
    def test_saved_schema_matches_current_feature_code(self):
        ...
        self.assertEqual(
            self.schema["moisture_model_features"], MOISTURE_FEATURE_NAMES
        )
    ...
    def test_reloaded_quality_pipeline_predicts_a_valid_row(self):
        process_snapshot = make_window(n_rows=1).iloc[-1]
        features = compute_moisture_features(
            process_snapshot,
            product_density=1.025,
            final_product_temp=43.0,
        )
        frame = pd.DataFrame([features], columns=MOISTURE_FEATURE_NAMES)
    ...
    def test_training_report_metrics_are_finite(self):
        ...
        test_metrics = report["quality"]["moisture"]["metrics"]["test"]
        for key in ("mae", "rmse", "r2", "bias", "max_abs_error"):
            self.assertTrue(np.isfinite(test_metrics[key]), f"moisture.{key}")
...
class RealtimeServiceHelperTests(unittest.TestCase):
    ...
    def _artifacts_stub(self, prediction=1.83):
        ...
        return self.service.Artifacts(
            ...
            feature_schema={
                ...
                "moisture_model_features": MOISTURE_FEATURE_NAMES,
    ...
    def test_prediction_produced_for_a_valid_row_with_history(self):
        ...
        buffer["Product Density"] = np.nan
        buffer["Final Product Temp"] = np.nan
        buffer.loc[10, "Product Density"] = 1.025
        buffer.loc[10, "Final Product Temp"] = 43.0
    ...
    def test_insufficient_history_yields_no_prediction_with_reason(self):
        ...
        self.assertIn("residence-adjusted time", quality.reason)
