...
| Path | Purpose |
|---|---|
| `powerbi_dashboard/MAP Dryer AI Dashboard.pbip` | Project entry point |
| `powerbi_dashboard/MAP Dryer AI Dashboard.SemanticModel/` | TMDL semantic model: `definition.pbism`, `.platform`, `definition/{database,model,expressions,relationships}.tmdl`, `definition/tables/vw_dryer_dashboard_powerbi.tmdl`, `definition/tables/vw_dryer_contributors_powerbi.tmdl` |
| `powerbi_dashboard/MAP Dryer AI Dashboard.Report/` | PBIR report: `definition.pbir`, `.platform`, `definition/{report,version}.json`, `definition/pages/pages.json`, 2 `page.json`, 37 `visual.json`, OCP industrial theme + base theme |
| `powerbi_dashboard/sql/create_powerbi_views.sql` | Reference DDL — run only if a view is missing |
...
| `powerbi_dashboard/README.md` | Operating / connection / publishing guide |
| `powerbi_dashboard/IMPLEMENTATION_REPORT.md` | This report |
...
* `powerbi_dashboard/sql/bootstrap_base_schema.sql` (**new**) — the base
  ...
  the 5-s migration).
* `powerbi_dashboard/sql/upgrade_5s_schema.sql` — extends
...
→ `powerbi_dashboard/preview/*.html` + PNG screenshots for visual
...
`powerbi_dashboard/preview/preview_page1_overview.{html,png}` and
